...
$DraftRecord = Get-Content -Raw $Draft | ConvertFrom-Json
...
if ($DraftRecord.importSucceeded) {
  $Answers.reconciliationAttempted = Ask-YesNo 'Did you reconcile CafeOS totals against the source POS/report?'
} else {
  $Answers.reconciliationAttempted = $false
}

$Answers.metricTrusted = if ($DraftRecord.importSucceeded -and $Answers.reconciliationAttempted) {
  Ask-YesNo 'After reconciliation, are the metrics trusted?'
} else {
  $false
}

$Answers.insightReviewed = if ($DraftRecord.importSucceeded) {
  Ask-YesNo 'Did the operator review at least one CafeOS insight?'
} else {
  $false
}

$Answers.usefulNewInsight = if ($Answers.metricTrusted -and $Answers.insightReviewed) {
  Ask-YesNo 'Was at least one insight useful AND new to the operator?'
} else {
  $false
}

$Answers.repeatUseAsked = Ask-YesNo 'Did you ask whether they want to use it again?'
$Answers.repeatUseIntent = if ($Answers.repeatUseAsked) {
  Ask-YesNo 'Did they want repeat use?'
} else {
  $false
}

$Answers.continuousSyncAsked = Ask-YesNo 'Did you ask whether they want continuous POS sync?'
$Answers.continuousSyncIntent = if ($Answers.continuousSyncAsked) {
  Ask-YesNo 'Did they want continuous sync?'
} else {
  $false
}

$Answers.valueDemonstrated = Ask-YesNo 'Was concrete product value demonstrated before the WTP question?'
$Answers.wtpAsked = if ($Answers.valueDemonstrated) {
  Ask-YesNo 'Did you ask willingness to pay?'
} else {
  $false
}
$Answers.willingnessToPay = if ($Answers.wtpAsked) {
  Ask-YesNo 'Did they express willingness to pay?'
} else {
  $false
}

if ($Answers.willingnessToPay) {
  $Allowed = @('<500k','500k-1m','1m-2m','2m+')
  while ($true) {
    $Band = (Read-Host 'Optional paid WTP band: <500k / 500k-1m / 1m-2m / 2m+ (press Enter to skip)').Trim()
    if (-not $Band) { break }
    if ($Allowed -contains $Band) { $Answers.wtpBand = $Band; break }
    Write-Host 'Use one of the listed paid bands, or press Enter to skip.' -ForegroundColor Yellow
  }
}
