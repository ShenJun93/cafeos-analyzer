...
function clockPartsAt(instantMs, timeZone) {
...
function zonedClockToDate(parts, timeZone) {
    const zone = assertValidTimeZone(timeZone);
...
export function localBusinessClock(occurredAt, timeZone) {
    const zone = assertValidTimeZone(timeZone);
    const date = occurredAt instanceof Date ? occurredAt : new Date(occurredAt);
    if (Number.isNaN(date.valueOf()))
        throw new Error("Invalid occurredAt instant");
...
export function daypartForInstant(occurredAt, timeZone) {
...
export function previousSameWeekdayDates(date, count = 4) {
