...
function parseCentralDirectory(data) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (uncompressedSize > MAX_ENTRY_BYTES)
            throw new Error(`XLSX entry too large: ${name}`);
        ...
        if (totalUncompressed > MAX_TOTAL_UNCOMPRESSED)
            throw new Error("XLSX uncompressed content exceeds limit");
...
function readEntry(data, entry) {
    ...
    if (end > data.length)
        throw new Error(`ZIP entry exceeds file bounds: ${entry.name}`);
...
function textNodes(xml) {
    ...
    while ((match = re.exec(xml)))
        out += xmlDecode(match[1].replace(/<[^>]+>/g, ""));
    return out;
}
...
function parseWorkbookDefinition(xml) {
...
function parseRelationships(xml) {
    ...
    while ((match = re.exec(xml))) {
        ...
        if (attrs.Id && attrs.Target)
            result.set(attrs.Id, normalizeTarget(attrs.Target));
    }
    return result;
}
...
function parseDateStyleIndexes(stylesXml) {
    ...
    while ((match = fmtRe.exec(stylesXml))) {
        ...
        if (Number.isFinite(id) && looksLikeDateFormat(attrs.formatCode ?? ""))
...
function cellValue(xml, attrs, shared, dateStyles, date1904) {
...
function parseWorksheet(xml, shared, dateStyles, date1904) {
    ...
    while ((rowMatch = rowRe.exec(xml))) {
        if (rows.length >= MAX_ROWS_PER_SHEET)
            throw new Error(`Worksheet exceeds ${MAX_ROWS_PER_SHEET} row limit`);
        const row = [];
...
function parseWorkbook(data) {
    if (data.byteLength > MAX_XLSX_BYTES)
        throw new Error(`XLSX exceeds ${MAX_XLSX_BYTES / 1024 / 1024}MB limit`);
    const entries = parseCentralDirectory(data);
    const readXml = (name, optional = false) => {
    ...
    if (workbook.sheets.length > MAX_SHEETS)
        throw new Error(`Workbook exceeds ${MAX_SHEETS} sheet limit`);
    ...
    for (const sheet of workbook.sheets) {
        ...
        if (!target || !target.startsWith("xl/worksheets/"))
            continue;
...
const REQUIRED_FIELDS = ["transaction_id", "occurred_at", "store", "product", "quantity", "net_amount"];
function inspectRows(name, rows) {
...
export function inspectXlsx(data) {
...
export function analyzeXlsx(data, sheetName, mappingOverride, options = {}) {
    ...
    if (!selectedName)
        throw new Error("No worksheet maps all required CafeOS fields");
    if (!sheetName && ambiguous)
        throw new Error("Multiple equally plausible transaction sheets; select a sheet explicitly");
