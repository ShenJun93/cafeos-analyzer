...
function priorWeekDate(date, weeks) {
    const d = new Date(`${date}T00:00:00.000Z`);
    d.setUTCDate(d.getUTCDate() - (weeks * 7));
    return d.toISOString().slice(0, 10);
}
export function detectStoreDaypartDeclines(items, threshold = -0.2, options = { defaultStoreTimezone: "UTC" }) {
    const defaultStoreTimezone = assertIanaTimeZone(options.defaultStoreTimezone);
    const storeZones = new Map();
    const timezoneForStore = (store) => {
    ...
    for (const item of items) {
        ...
        const b = buckets.get(key) ?? { store: item.store, part, date, sales: 0, orders: new Set() };
    ...
    for (const [key, points] of groups) {
        ...
        const [store, part] = JSON.parse(key);
        result.push({
