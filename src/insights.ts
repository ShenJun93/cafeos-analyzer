...
export function detectStoreDaypartDeclines(items: CanonicalLineItem[], threshold = -0.2): Insight[] {
  ...
  for (const item of items) {
    ...
    const b = buckets.get(key) ?? { sales: 0, orders: new Set<string>() };
    b.sales += item.netAmount; b.orders.add(item.transactionId); buckets.set(key, b);
  }
  ...
  for (const [key, value] of buckets) {
    ...
    g.push({ date, sales: value.sales, orders: value.orders.size }); groups.set(`${store}|${part}`, g);
  }
