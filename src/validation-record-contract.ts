...
export const PARTICIPANT_ROLE_CLASSES = ['owner_operator', 'other'] as const;
export type ParticipantRoleClass = typeof PARTICIPANT_ROLE_CLASSES[number];

export const STORE_BUCKETS = ['1', '2', '3-5', '6-10', '11-15', '16+'] as const;
export type StoreBucket = typeof STORE_BUCKETS[number];

export const PRIVACY_MODES = ['local-only', 'profile', 'pseudonymized', 'permissioned-raw'] as const;
export type PrivacyMode = typeof PRIVACY_MODES[number];

export const WTP_BANDS = ['0', '<500k', '500k-1m', '1m-2m', '2m+'] as const;
export type WtpBand = typeof WTP_BANDS[number];

export const ACQUISITION_SOURCES = ['seo', 'direct', 'other'] as const;
export type AcquisitionSourceClass = typeof ACQUISITION_SOURCES[number];

export const ACQUISITION_MEDIA = ['organic', 'none', 'other'] as const;
...
export function deriveAnalyzerStoreBucket(storeCount: number): StoreBucket {
...
export function deriveAnalyzerTargetIcp(
  participantRoleClass: ParticipantRoleClass,
  storeCount: number
): boolean {
...
export function normalizeAnalyzerSource(raw: string | undefined | null): AnalyzerSourceClass {
  const value = String(raw ?? '').trim().toLowerCase().replace(/\s+/g, '_');
  if (!value) return 'unknown';
  if (value === 'kiotviet') return 'kiotviet';
  if (value === 'cukcuk') return 'cukcuk';
  if (value === 'sapo_fnb' || value === 'sapo-fnb' || value === 'sapo') return 'sapo_fnb';
  if (value === 'ipos') return 'ipos';
  if (value === 'pos365') return 'pos365';
  if (
    value === 'generic_excel_csv' ||
  ...
  if (value === 'other_pos' || value === 'other-pos' || value === 'other') return 'other_pos';
...
export function normalizeAcquisitionAttribution(
  ...
  const knownCampaigns = new Set<AcquisitionCampaignClass>([
    'kiotviet-excel-analysis',
    'sapo-fnb-excel-analysis',
    'cafe-excel-analysis'
  ]);
  if (s === 'seo' && m === 'organic' && knownCampaigns.has(c as AcquisitionCampaignClass)) {
    return {
      acquisitionSource: 'seo',
      acquisitionMedium: 'organic',
      acquisitionCampaign: c as AcquisitionCampaignClass
    };
  }
