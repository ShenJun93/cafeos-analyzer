...
export function assertIanaTimeZone(timeZone: string): string {
  ...
  if (!zone) throw new Error("IANA timezone is required");
  try {
...
export function zonedDateTimeParts(
  ...
  const result: Partial<LocalDateTimeParts> = {};
...
function validCalendarParts(parts: LocalDateTimeParts): boolean {
...
export function wallClockToInstant(parts: LocalDateTimeParts, timeZone: string): Date {
  const zone = assertIanaTimeZone(timeZone);
  if (!validCalendarParts(parts)) throw new Error("Invalid local date/time");
...
export function parseSourceTimestamp(value: string, sourceTimezone?: string): Date | null {
  ...
  if (vn) {
    ...
    if (!sourceTimezone) throw new Error("sourceTimezone is required for naive timestamps");
    try {
...
export function daypartForInstant(value: Date | string, timeZone: string): "morning" | "afternoon" | "evening" {
